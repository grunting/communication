...
public class GroupImpl2 implements Group {
    ...
    public byte[] create01(String name,byte[] crypto) throws Exception {
        Set<Friend> friends = ReportImpl2.index.getNode("names",name);
        ...
        crypto = RSA.encrypt(tar.getBytes(),client.getKeyPair().getPrivate());
    ...
    public static boolean createGroup(String name) throws Exception {
        ...
        synchronized (name) {
            ...
            GroupServer groupServer = client.getRemoteProxyObj(GroupServer.class);

            // 所以会产生负数?
            int i1 = random.nextInt(Integer.MAX_VALUE) - 1;

            // 用自身的秘钥发送随机数,证明自身
            byte[] crypto = RSA.encrypt(String.valueOf(i1).getBytes(),client.getKeyPair().getPrivate());
            ...
            if (Integer.parseInt(tar[0]) - i1 == 1) {
                ...
                // 再次发送,用对方发送过来的数字+1发送
                crypto = RSA.encrypt(String.valueOf(i1).getBytes(),client.getKeyPair().getPrivate());

                // 获取对称秘钥
                crypto = groupServer.createGroup02(name,crypto);

                real = RSA.decrypt(crypto,client.getKeyPair().getPrivate());
                if (real.length != 0) {
                    Passageway passageway = new Passageway();
                    passageway.aes = new AES(real);
                    passageWays.put(name,passageway);
